...
The following table shows the main attributes available for `wsHealthNotification*` events:
...
```sql
SELECT uniqueCount(affectedEntityArn) FROM AwsHealthNotification 
WHERE statusCode = 'open' AND eventTypeCategory = 'Issue' AND service = 'EC2'
```
